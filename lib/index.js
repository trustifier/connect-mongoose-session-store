...
function _initialize (dbConfig) {
  ...
  Mongoose.connection.on('connected', function () {
    console.log('Session Store connected to ' + dbURI);
  });

  Mongoose.connection.on('error', function (error) {
    console.log('Session Store connected error ' + error);
  ...
  process.on('SIGINT', function () {
    Mongoose.connection.close(function () {
      console.log('Session Store disconnected through app termination');
      process.exit(0);
    });
  });
}
  ...
  SessionStore.prototype.set = function (sid, session, next) {
    ...
    try {
      ...
      if (maxAge > session.cookie.originalMaxAge) {
        console.log("Session store max age should be lower than the session cookie max age. Authentication failed.");
